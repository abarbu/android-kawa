...
(define-simple-class StatusActivity (BaseActivity <android.text.TextWatcher>)
 (edit-text     ::EditText)
 (update-button ::Button)
 (text-count    ::TextView)
 ((onCreate (savedInstanceState ::Bundle))
  (invoke-special Activity (this) 'onCreate savedInstanceState)
  (invoke-special Activity (this) 'setContentView R$layout:status)
  (set! edit-text 
	(invoke-special Activity (this) 'findViewById R$id:editText))
  (set! update-button
	(invoke-special Activity (this) 'findViewById R$id:buttonUpdate))
  (update-button:setOnClickListener
   (lambda ((view ::View))
    ;; FIXME this is nasty
    (let* ((activity-this (this)) (application (this):yamba))
     (*:execute
      (async-task
       String int String
       (background statuses
		   (try-catch
		    (begin 
		     ((application:get-twitter):setStatus (statuses 0))
		     (statuses 0))
  ...
  (set! text-count (invoke-special Activity (this) 'findViewById R$id:textCount))
	 ...
	 (try-catch
	  (let loop ()
	   (when running?
	    ...
	    (loop)))
	  (e java.lang.InterruptedException #f)))))
...
(define (for-each-cursor! (f ::procedure) (cursor ::Cursor))
 (let loop () (when (cursor:moveToNext) (f cursor) (loop))))

(define-simple-class TimelineActivity (BaseActivity)
 (cursor	::Cursor)
 (listTimeline  ::ListView)
 (adapter       ::SimpleCursorAdapter)
 ((onCreate (savedInstanceState ::Bundle))
  (invoke-special BaseActivity (this) 'onCreate savedInstanceState)
  (invoke-special Activity (this) 'setContentView R$layout:timeline)
  (when (equal? #!null ((this):yamba:preferences:getString "username" #!null))
   (invoke-special Activity (this) 'startActivity
		   (make <android.content.Intent> (this) PrefActivity))
   ((Toast:makeText (this) R$string:msgSetupPrefs Toast:LENGTH_LONG):show))
  (set! listTimeline (invoke-special Activity (this) 'findViewById R$id:listTimeline)))
 ((onDestroy)
  (invoke-special BaseActivity (this) 'onDestroy)
  (((this):yamba:get-status-data):close))
 ((onResume)
  (invoke-special BaseActivity (this) 'onResume)
  (set! cursor (((this):yamba:get-status-data):get-status-updates))
  (invoke-special Activity (this) 'startManagingCursor cursor)
  (set! adapter (make SimpleCursorAdapter
		 (this) R$layout:row cursor
		 (String[] StatusData:C_CREATED_AT
			StatusData:C_USER
			StatusData:C_TEXT)
		 (int[] R$id:textCreatedAt R$id:textUser R$id:textText)))
  (adapter:setViewBinder
   (lambda ((view ::View) (cursor ::Cursor) (columnIndex ::int))
    (if (not (equal? (view:getId) R$id:textCreatedAt))
	#f
	(begin
	 ((as TextView view):setText
	  (DateUtils:getRelativeTimeSpanString
	   (view:getContext) (cursor:getLong columnIndex)))
	 #t))))
  (listTimeline:setAdapter adapter)))

(define-class BaseActivity (Activity)
 interface: #f
 (yamba ::YambaApplication)
 ((onCreate (savedInstanceState ::Bundle))
  (invoke-special android.app.Activity (this) 'onCreate savedInstanceState)
  (set! yamba (as YambaApplication (getApplication))))
 ...
 ((onOptionsItemSelected (item ::MenuItem))
  (cond ((equal? (item:getItemId) R$id:itemPrefs)
	...
	((equal? (item:getItemId) R$id:itemToggleService)
	 (if yamba:service-running?
	     (stopService (make <android.content.Intent> (this) UpdaterService))
	     (startService (make <android.content.Intent> (this) UpdaterService))))
	((equal? (item:getItemId) R$id:itemPurge)
	 ((yamba:get-status-data):delete)
	 ((Toast:makeText (this) R$string:msgAllDataPurged Toast:LENGTH_LONG):show))
	((equal? (item:getItemId) R$id:itemTimeline)
	 (startActivity
	  (((make <android.content.Intent> (this) TimelineActivity):addFlags
	    Intent:FLAG_ACTIVITY_SINGLE_TOP):addFlags
	    Intent:FLAG_ACTIVITY_REORDER_TO_FRONT)))
	((equal? (item:getItemId) R$id:itemStatus)
	 (startActivity
	  ((make <android.content.Intent> (this) StatusActivity):addFlags
	   Intent:FLAG_ACTIVITY_REORDER_TO_FRONT))))
  #t)
 ((onMenuOpened (featureId ::int) (menu ::Menu))
  (let ((toggleItem (menu:findItem R$id:itemToggleService)))
   (if yamba:service-running?
       (begin (toggleItem:setTitle R$string:titleServiceStop)
	      (toggleItem:setIcon android.R$drawable:ic_media_pause))
       (begin (toggleItem:setTitle R$string:titleServiceStart)
	      (toggleItem:setIcon android.R$drawable:ic_media_play)))
   #t))
 ((onDestroy)
  (invoke-special Activity (this) 'onDestroy))
 ((onResume)
